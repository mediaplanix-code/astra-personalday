...
- generate-daily : 05:00 — genera oroscopi per tutti gli utenti profiles
- send-telegram  : 07:00 — push Telegram con link Luna
- check-trials   : 09:00 — scadenza trial

Sistema unificato su profiles — tabella clienti rimossa.
"""
...
async def generate_daily_horoscopes(request: Request, x_cron_secret: str = Header(None)):
    """
    Genera oroscopi personalizzati per tutti gli utenti attivi (solo profiles).
    Eseguito alle 05:00 — quando l'utente apre il sito, l'oroscopo è già pronto.
    """
    ...
    for user in users:
        try:
            # Salta se oroscopo già generato oggi
            ...
            # Tema natale completo — case + aggiornamento ascendente/luna se mancano
            ...
            if natal:
                ...
                # Salva pianeti e case se non già presenti
...
async def send_telegram_push(request: Request, x_cron_secret: str = Header(None)):
    """
    Invia oroscopo del giorno via Telegram. Solo sistema profiles.
    Include link diretto a Luna per approfondire.
    Eseguito alle 07:00.
    """
    verify_cron_secret(x_cron_secret)
    ...
    for conn in connections:
        try:
            ...
            luna_url = f"https://astra-personal.pages.dev/?luna=1"
...
async def check_expired_trials(request: Request, x_cron_secret: str = Header(None)):
    """
    Porta i trial scaduti a 'expired'. Solo sistema profiles/subscriptions.
    Eseguito alle 09:00.
    """
    verify_cron_secret(x_cron_secret)
