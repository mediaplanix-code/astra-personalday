...
async def get_planetary_positions(birth_lat: float, birth_lng: float, birth_date: str, birth_time: str, timezone: str, settings) -> dict:
    """Chiama nocturna-calculations per ottenere posizioni planetarie."""
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.post(
                f"{settings.nocturna_api_url}/api/v1/calculations/natal",
...
async def get_transits_today(birth_lat: float, birth_lng: float, settings) -> dict:
    """Recupera transiti del giorno da nocturna-calculations."""
    try:
        today = date.today().isoformat()
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.post(
                f"{settings.nocturna_api_url}/api/v1/calculations/transits",
                headers={"Authorization": f"Bearer {settings.nocturna_service_token}"},
                json={
                    "date": today,
                    "time": "12:00",
