...
async def save_birth_data(request: Request, body: BirthDataUpdate):
    """
    Salva dati natali. Calcola segno solare, ascendente e luna immediatamente.
    ...
    # ── Calcolo tema natale completo (ascendente + luna) ──────────
    # Eseguito subito alla registrazione: le chips sono sempre popolate
    if body.birth_date and body.birth_lat and body.birth_lng:
        try:
            ...
            natal = get_natal_chart(
                ...
                timezone=body.birth_timezone or "Europe/Rome"
            )
            if natal:
            ...
            logging.getLogger(__name__).warning(
                f"Calcolo natale fallito in save_birth_data: {e}"
            )
            # Non blocca il salvataggio — calcolati al primo oroscopo
    ...
    return {
        "ok": True,
        "sun_sign":  update.get("sun_sign"),
        "ascendant": update.get("ascendant"),
        "moon_sign": update.get("moon_sign"),
    }
